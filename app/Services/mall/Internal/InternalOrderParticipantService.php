...
/**
 * Saga order step: bind remote inventory token to an existing draft bet.
 */
final class InternalOrderParticipantService
{
    ...
    public function confirmPaid(int $orderId): BetOrder
    {
        $order = $this->orders->findById($orderId);
        ...
        return $this->orders->transitionStatus($order, BetOrderStatus::Accepted, false);
    }

    public function compensate(int $orderId): BetOrder
    {
        $order = $this->orders->findById($orderId);

        return $this->orders->transitionStatus($order, BetOrderStatus::Cancelled, false);
    }
}
