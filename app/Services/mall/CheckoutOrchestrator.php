...
/**
 * Step 2: POST /api/bet/checkout — saga start drives selection "inventory", order bind, TCC (points + prepay), then assembles client prepay.
 */
final readonly class CheckoutOrchestrator
{
    ...
    /**
     * @return array{order: BetOrder, prepay: array<string, mixed>, points_tcc_idem_key: string|null, tid: string}
     */
    public function checkoutExistingOrder(int $uid, BetOrder $order, int $pointsMinor, string $xRequestId = '0'): array
    {
        if ($order->uid !== $uid) {
        ...
        $flowId = (int) config('bet_agg.saga.flow_id', 0);
        $accessKey = trim((string) config('bet_agg.saga.access_key', ''));
        if ($flowId < 1 || $accessKey === '') {
            throw new RuntimeException(
                'Saga coordinator is not configured. Set BET_SAGA_FLOW_ID and BET_SAGA_ACCESS_KEY.'
            );
        }

        $tccFlowId = (int) config('bet_agg.tcc.flow_id', 0);
        if ($tccFlowId < 1) {
            throw new RuntimeException(
                'TCC is not configured for checkout. Set BET_TCC_FLOW_ID.'
            );
        }

        $stepCodes = config('bet_agg.saga.checkout_steps');
        $branchCodes = config('bet_agg.tcc.checkout_branches');
        if (! is_array($stepCodes) || ! is_array($branchCodes)) {
            throw new RuntimeException('bet_agg saga.checkout_steps / tcc.checkout_branches must be configured.');
        }
        ...
        if ($inventoryStep === '' || $orderStep === '' || $payStep === '' || $tryPointsBranch === '' || $prepayBranch === '') {
            throw new RuntimeException('Checkout step or TCC branch code resolved empty; check bet_agg config.');
        }
     ...
     */
    private function assembleCheckoutPrepay(array $partial, BetOrder $order, int $uid): array
    {
        $order->refresh();
        ...
        $envelope = [
            ...
            'uid' => $uid,
            'amount_minor' => $cash,
            'invoke_payment' => $cash > 0 ? 'placeholder' : 'none',
        ];
