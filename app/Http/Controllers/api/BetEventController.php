...
class BetEventController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $pack = $this->catalog->listOpenEvents($page, $perPage);
        $this->logHandledApiRequest($request, ['handler' => 'bet.events.index']);
    ...
    {
        try {
            $row = $this->catalog->getEventDetail($event_id);
        } catch (\RuntimeException $e) {
            return response()->json(ApiResponse::error(40401, $e->getMessage()), 404);
        }

        $this->logHandledApiRequest($request, ['handler' => 'bet.events.show', 'event_id' => $event_id]);
