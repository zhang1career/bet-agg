...
class UserAggregationController extends Controller
{
    public function me(
        ...
        $responseCode = $hasDegraded
            ? (int) config('bet_agg.degrade.partial_failure_code', 20601)
            : 0;
        $responseMsg = $hasDegraded
            ? (string) config('bet_agg.degrade.partial_failure_message', 'Partially failed, degraded by aggregator.')
            : '';
        ...
        return response()->json(ApiResponse::code([
            ...
            'meta' => [
                ...
                'execution_mode' => (string) config('bet_agg.execution.mode', 'serial'),
            ],
