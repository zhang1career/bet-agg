...
class LogApiHttpErrors
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        if (config('bet_agg.api.log_http_errors', true)) {
        ...
        if (
            $status >= 500
            && config('bet_agg.api.normalize_5xx_json_body', false)
        ) {
            $message = (string) config('bet_agg.api.normalize_5xx_message', '服务器内部错误');
