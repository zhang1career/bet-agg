...
@section('content')
    ...
    {{-- Settlement --}}
    @if($settlementForm)
        @php($settlementGames = $settlementForm['games'])
        @php($gameSelectLabels = $settlementForm['gameSelectLabels'])
        @php($outcomesByGame = $settlementForm['outcomesByGame'])
        <div class="modal fade" id="mallModalSettlement" tabindex="-1" aria-hidden="true"
             data-mall-modal="1"
             data-mall-strip-query="mall_settlement"
             data-mall-auto-show="1">
            <div class="modal-dialog modal-dialog-scrollable">
                <div class="modal-content">
                    <form method="post" action="{{ route('admin.settlement.store') }}">
                        @csrf
                        <div class="modal-header">
                            <h2 class="modal-title h5">录入赛果（入队结算）</h2>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <p class="text-muted small">提交后写入队列，由 Worker 执行兑奖。</p>
                            <div class="mb-3">
                        ...
                        <div class="modal-footer">
                            <a href="{{ route('admin.games.index') }}" class="btn btn-outline-secondary">取消</a>
                            <button type="submit" class="btn btn-primary" @if($settlementGames->isEmpty()) disabled @endif>入队结算</button>
                        </div>
                    </form>
                </div>
            </div>
