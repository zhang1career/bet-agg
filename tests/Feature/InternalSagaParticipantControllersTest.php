...
final class InternalSagaParticipantControllersTest extends TestCase
{
    ...
    public function test_order_action_binds_draft_order_to_inventory_token(): void
    {
        ...
        $order = app(OrderCommandService::class)->createDraftPendingOrder(7, [['selection_id' => $sid, 'stake_points' => 1]]);

        $invIdem = 'inv-'.bin2hex(random_bytes(3));
        $tryInv = $this->postJson('/internal/inventory/action', [
            'payload' => [
                'uid' => 7,
                'lines' => [['product_id' => $sid, 'quantity' => 1]],
    ...
    public function test_pay_try_creates_prepay_stub(): void
    {
        ...
        $sid = SportSeeder::openSelection(2000);
        $order = app(OrderCommandService::class)->createDraftPendingOrder(3, [['selection_id' => $sid, 'stake_points' => 1]]);
    ...
    {
        $sid = SportSeeder::openSelection(2000);
        $order = app(OrderCommandService::class)->createDraftPendingOrder(4, [['selection_id' => $sid, 'stake_points' => 1]]);
        $order->cash_payable_minor = (int) $order->total_price;
        $order->save();
